...
object StreamApp {
  def main(args: Array[String]): Unit = {
    ...
    val kafkaParams = Map[String, Object](
      ...
      "enable.auto.commit" -> (false: java.lang.Boolean) // 禁用自动提交Offset，否则可能没正常消费完就提交了，造成数据
    )

    // KafkaUtils.createDirectStream()
    //val kafkaManager = new KafkaManager(zkQuorum, kafkaParams)
    val directStream: InputDStream[ConsumerRecord[String, String]] = null //= kafkaManager.createDirectStream(ssc, topics)

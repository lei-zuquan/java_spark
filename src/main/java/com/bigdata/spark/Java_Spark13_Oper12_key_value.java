 ...
 * @Date: Created in 2:49 下午 2020/3/1
 * @Version: 1.0
 * @Modified By:
 * @Description:
 */
public class Java_Spark13_Oper12_key_value {
...
//        /**
//         * 第一个是初始值（看需求，第二个参数，表示两两求最大值。
//         * var aggrRDD: RDD[(String, Int)] = listRDD.aggregateByKey(0)(math.max(_, _), _ + _)
//         */
//
//        // 这里其实就是分区内相加，分区间也相加，就是求wordCount
//        var aggrRDD: RDD[(String, Int)] = listRDD.aggregateByKey(0)(_ + _, _ + _)
//
//        // 这里比aggregateByKey 少了一个参数，表示分区内 和 分区间的算法可以使用一样的, 是对上面的简化
//        var foldRDD: RDD[(String, Int)] = listRDD.foldByKey(0)(_ + _)
//
//        // 使用combineByKeyWithClassTag 计算平均值，没有初始值，但是要变成 （key,value）_1，再次有分区内和分区间
//        var combine: RDD[(String, (Int, Int))] =
//        listRDD.combineByKeyWithClassTag((_, 1), (acc: (Int, Int), v)
//      => (acc._1 + v, acc._2 + 1),
